...
class Post {
    ...
    private $uniquePost, $uniqueUser, $title, $content, $postTime, $tags, $visibility, $allowComments;

    public function __construct($uniquePost)
    {
        $this->uniquePost = $uniquePost;
        global $sql;
        $checkUnique = $sql->query("SELECT `unique_user`,`title`,`content`,`post_time`,`tags`,`visibility`,`allow_coomments` FROM `posts` WHERE `unique_post`='" . $this->uniquePost . "'");
    ...
    public static function add($uniqueUser, $title, $content, $tags, $visibility, $allowComments)
    {
        ...
        {
            try {
                $sql->query("INSERT INTO `posts` (`unique_post`,`unique_user`,`title`,`content`,`post_time`,`tags`,`visibility`,`allow_comments`) VALUES ('" . $uniquePost . "','" . $uniqueUser . "','" . $title . "','" . $content . "','" . date("Y-m-d H:i:s") . "','" . $tags . "','" . $visibility . "','" . $allowComments . "')");
